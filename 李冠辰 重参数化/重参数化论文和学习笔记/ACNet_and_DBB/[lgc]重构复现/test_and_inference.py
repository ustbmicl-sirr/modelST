...
# ========= 测试等价转换后的模型 =========
test_model_dbb = DbbCNNnet()
test_model_dbb.load_state_dict(torch.load("./mynet.pth"))
for m in test_model_dbb.modules():  # 等价转换
    if hasattr(m, 'switch_to_deploy'):
        m.switch_to_deploy()
test_model_dbb.eval()
test_model_dbb.cuda()
test(test_model_dbb, get_loader()[1])
