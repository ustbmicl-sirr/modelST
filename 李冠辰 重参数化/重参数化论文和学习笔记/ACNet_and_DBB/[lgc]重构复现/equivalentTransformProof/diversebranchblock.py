...
class IdentityBasedConv1x1(nn.Conv2d):
    """
    使用IdentityBasedConv1x1替代一个普通的1×1卷积
        这个类继承了nn.Conv2d, 限定kernel=1; stride=1, 支持分组卷积
        每个分组的卷积核都进行对角线增强: 新卷积核 = 旧卷积核 + 旧卷积核同型的单位矩阵 
    """
...
class BNAndPadLayer(nn.Module):
    """
    BNAndPadLayer: 使用BN层超参数偏移对输出特征图进行padding
    """
...
class DiverseBranchBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if deploy:
            ...
            # 第二个分支: 1×1卷积 + BN + AvgPooling + BN
            self.dbb_avg = nn.Sequential()
            if groups < out_channels:
                self.dbb_avg.add_module('conv', nn.Conv2d(
                    in_channels=in_channels, out_channels=out_channels,
                    kernel_size=1, stride=1, padding=0, groups=groups, bias=False
                ))
            ...
            # 第四个分支: 1×1卷积 + BN + k×k卷积 + BN
            self.dbb_1x1_kxk = nn.Sequential()
            if internal_channels_1x1_3x3 == in_channels:
                self.dbb_1x1_kxk.add_module('idconv1', IdentityBasedConv1x1(channels=in_channels, groups=groups))
            else:
                self.dbb_1x1_kxk.add_module('conv1', nn.Conv2d(
                    in_channels=in_channels, out_channels=internal_channels_1x1_3x3,
                    kernel_size=1, stride=1, padding=0, groups=groups, bias=False
                ))
            self.dbb_1x1_kxk.add_module('bn1', BNAndPadLayer(
                pad_pixels=padding, num_features=internal_channels_1x1_3x3, affine=True
            ))
            self.dbb_1x1_kxk.add_module('conv2', nn.Conv2d(
                in_channels=internal_channels_1x1_3x3, out_channels=out_channels,
                kernel_size=kernel_size, stride=stride, padding=0, groups=groups, bias=False
            ))
            self.dbb_1x1_kxk.add_module('bn2', nn.BatchNorm2d(out_channels))

        # 自定义初始化有时效果要好于默认初始化, 可供尝试 
        if single_init:
            self.single_init()

    def get_equivalent_kernel_bias(self):
        """
        将四个分支融合成一个分支, 形成一个k×k卷积核参数 + k×k卷积核偏置
        """
    ...
    def switch_to_deploy(self):
        """
        修改网络结构: 建立self.dbb_reparam, 删除dbb_origin, dbb_avg, dbb_1x1, dbb_1x1_kxk
        """
